...
// globals
const { numOfHashes, defaultAvatar, allowedAvatarTypes } = require('../config/globals.js');
...
router.put('/avatar/:user_id', authenticate, fileUpload(), (req, res) => {
  const { user_id } = req.params;
	...
	if (!allowedAvatarTypes.includes(mimeType)) {
		return res.status(401).json({ error: `${ mimeType.replace('image/', '') } is not an allowed avatar type. It must be a jpeg, jpg, png, bmp, or tiff.` });
	}
	return Jimp.read(imageBuffer)
		.then(image => {
			return image.scaleToFit(100, 100)
