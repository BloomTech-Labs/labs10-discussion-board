...
const { secureKey, accountUserTypes, addCatPermStartIndex } = require('../globals.js');
...
const authorizeCreateCat = async (req, res, next) => {
  ...
  try {
    await jwt.verify(token, secureKey, async (err, decoded) => {
      ...
      const userSettings = await usersDB.getUserType(loggedInUserID);
      if (accountUserTypes.indexOf(userSettings.user_type) < addCatPermStartIndex) throw { code: 403 };
      next();
    });
  } catch (err) {
    if (err.code === 401) {
